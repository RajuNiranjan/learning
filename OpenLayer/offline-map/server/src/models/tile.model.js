    ...
    try {
        const result = await pool.query(
           `INSERT INTO tile_table (name, extent, center, zoom, projection, thubmnailBase64Img)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *`,
            [tileData.name, tileData.extent, tileData.center, tileData.zoom, tileData.projection, tileData.thumbnailBase64Img]
        )
...
export const getTileByIdService = async (tileId) => {
    try {
        const result = await pool.query(
            'SELECT * FROM tile_table WHERE id = $1',
            [tileId]
        );
        return result.rows[0];
    } catch (error) {
...
export const deleteTileService = async (tileId) => {
    try {
        const result = await pool.query(
            'DELETE FROM tile_table WHERE id = $1',
            [tileId]
        )
        return result.rows[0]
    } catch (error) {
